...
|---|---|---|---|
| **Métricas, taxas, índices, inflação ou valores de destaque** | Painel de destaques quantitativos no topo ou seção analítica | **KPI Card Financeiro** (`.bcb-kpi-card`) | Grade flexível em 4 colunas (`.col-lg-3`) ou 3 colunas com microcopy label, valor em destaque Arial tabular bold (`var(--bcb-font-ubuntu)`), badge de tendência direcional com ícone (`.positive`, `.negative`, `.neutral`) e data de referência. |
...
| **Instruções passo a passo, etapas ordenadas ou fluxos sequenciais** | Roteiro cronológico visualmente numerado com instruções curtas | **Process List / Stepper** (`<ol class="process-list">` ou `.bcb-stepper`) | Lista ordenada com títulos semânticos `<h3>`, linha contínua sem quebras, textos objetivos e links diretos. |
| **Roteiros de decisão com alternativas (*Se Fraude* vs. *Se Desacordo*)** | Diagrama sequencial acessível em lista estruturada | **Fluxograma Textual com Decisão** (`.bcb-decision-flow`) | Lista ordenada acessível com nós generosos (`.bcb-decision-node`), bifurcações (`.bcb-decision-branches`), badges contrastantes "Sim" / "Não" (`.bcb-decision-badge--yes`, `.bcb-decision-badge--no`) e prazos legais destacados. |
...
| **Alertas regulatórios, prazos críticos, sanções ou dicas editoriais** | Caixa de destaque de alta visibilidade com contraste calibrado (WCAG AAA) | **Callout Oficial** (`.bcb-callout`) | `.bcb-callout.bcb-callout--info|warning|danger|success|editorial` com cabeçalho BEM, ícone semântico Material Symbols e corpo explicativo. A variante `--editorial` absorve a antiga Tip Box. |
...
| **Pronunciamentos de diretoria, atas do Copom ou artigos de resoluções** | Citação editorial institucional em grid WCAG AAA | **Citação / Destaque Normativo** (`.bcb-quote`) | Layout em grid com calha dedicada de ícone à esquerda, tipografia sem marcas sobrepostas e variante `.bcb-quote--normative`. |
...
| **Ações prioritárias, canais de segurança urgente ou conversão direta** | Bloco de chamada de alta visibilidade com botão de ação | **Call to Action — CTA** (`.bcb-cta`) | `.bcb-cta.bcb-cta--primary|neutral|centered` com tag semântica, título claro, descrição objetiva e botão `.bcb-btn-cta`. |
...
| **Notícias jornalísticas e artigos com lead e compartilhamento** | Cartão editorial com proporção 16:9 | **Card de Notícia Editorial** (`.bcb-news-card--editorial`) | Card com mídia 16:9, editoria, data ISO, manchete, lead explicativo e ações de compartilhamento. |
| **Acesso a sistemas, serviços rápidos e navegação temática** | Cards de atalho institucionais (Modelo 1 ou Modelo 2) | **Cards de Link** (`.bcb-card-link`) | `.bcb-card-link--modelo-1` (Lista de Links institucional) e `.bcb-card-link--modelo-2` (Card de destaque horizontal com thumbnail, tag, título e descrição). |
| **Apresentação de vídeos tutoriais e institucionais em lote** | Carrossel navegável por setas com suporte a scroll-snap | **Carrossel de Vídeos** (`.bcb-video-carousel`) | Header com botões direcionais, track de vídeos 16:9 com scroll suave e títulos com clamp de 2 linhas. |
| **Diálogos de confirmação, formulários modais e avisos focados** | Diálogo modal com focus-trap e calibragem de largura | **Modal Acessível** (`.bcb-modal`) | `.bcb-modal-backdrop` com `.bcb-modal-dialog` nas larguras `.bcb-modal--sm` (400px), `--md` (600px), `--lg` (850px), `--xl` (1140px). |
